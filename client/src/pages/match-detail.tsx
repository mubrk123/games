  ...
  return (
    <AppShell>
      <div className="space-y-4 pb-20">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              {match.status === 'FINISHED' && (
                <Badge variant="secondary" className="bg-gray-700">FINISHED</Badge>
              )}
              {wsConnected && (
                <Badge variant="outline" className="text-green-500 border-green-500">
                  <Wifi className="h-3 w-3 mr-1" /> Live
        ...
        {/* CRITICAL FIX: Check if match is finished before showing betting UI */}
        {match.status !== 'FINISHED' ? (
          <Tabs defaultValue={isCricket && match.status === 'LIVE' ? 'instance' : 'match-winner'} className="w-full">
            <TabsList className="w-full grid grid-cols-3 h-auto p-1">
              <TabsTrigger value="match-winner" className="text-[11px] py-2">
          ...
          // SHOW THIS WHEN MATCH IS FINISHED
          <Card className="bg-gray-900/50 border-gray-700">
            <CardContent className="p-6 text-center">
              <AlertCircle className="h-12 w-12 mx-auto text-gray-500 mb-4" />
              <h3 className="text-lg font-bold text-gray-300 mb-2">Match Ended</h3>
              <p className="text-gray-400 mb-4">{displayDetails || 'This match has finished'}</p>
