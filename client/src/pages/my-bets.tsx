...
export default function MyBets() {
  ...
  const openInstanceBets = instanceBets.filter((b: any) => b.status === 'OPEN');
  const settledInstanceBets = instanceBets.filter((b: any) => b.status !== 'OPEN');
  ...
  const InstanceBetCard = ({ bet }: { bet: any }) => {
    ...
    return (
      <div className="bg-card rounded-xl border border-border/50 p-4 space-y-3">
        ...
        <div>
          <p className="font-medium text-sm">{bet.outcomeName || bet.marketName}</p>
          <p className="text-xs text-muted-foreground">{bet.marketName}</p>
        </div>
      ...
      <div className="space-y-4">
        ...
        <Tabs defaultValue="open" className="w-full">
          <TabsList className="w-full grid grid-cols-3">
            ...
            <TabsTrigger value="instance" data-testid="tab-instance-bets">
              Instance ({instanceBets.length})
            </TabsTrigger>
          ...
          <TabsContent value="instance" className="mt-4 space-y-3">
            {isLoadingInstance ? (
              <div className="text-center py-8 text-muted-foreground">Loading...</div>
            ) : instanceBets.length === 0 ? (
              <div className="text-center py-12 text-muted-foreground">
                <p className="mb-2">No instance plays</p>
                <p className="text-xs">Ball-by-ball and over markets will appear here</p>
              </div>
            ) : (
              <>
